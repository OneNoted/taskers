...
wait_for_browser_state() {
  ...
  local socket_path=$2
  local status_path=$3
  local integrity_path=$4
  local expected_surface_id=$5
  local expected_url=$6
  local expected_title=$7
  local attempts=$((WAIT_TIMEOUT_SECONDS * 10))

  while (( attempts > 0 )); do
    "$control_bin" query status --socket "$socket_path" >"$status_path"
...
"$bundle_taskersctl" query status --socket "$socket_path" >"$status_path"
readarray -t ids < <(
...
printf '%s\n' 'Taskers launcher smoke passed: release bundle installed and loaded an embedded browser split.'
